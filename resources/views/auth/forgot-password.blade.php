                ...
                <form action="{{route('password.request')}}" method="POST">
                    ...
                    <div class="flex flex-col gap-2 mb-8">
                        <div class="inline-flex items-center gap-3">
                            <label class="text-left font-semibold inline" for="email">Email Address</label>
                            @error('email')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
                        <input class="border border-neutral-300 py-3 px-6 rounded-lg" type="text" name="email" placeholder="Email Address" value="{{ old('email') }}">
                    </div>
